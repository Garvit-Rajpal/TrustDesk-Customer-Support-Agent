...
6. `docs/HLD_v5.md` + `docs/LLD_v5.md` — v5 product extension (ADRs 25–29): shared visual/animation primitives, landing-page chat demo, testimonials carousel + footer, login/signup/dashboard deep visual redesign, `/portal/*` visual pass, magic-link customer-portal auth. Delta docs — v1/v2/v3/v4 remain valid where not amended.
7. `docs/ticket_lifecycle.mermaid` (v1), `docs/ticket_lifecycle_v2.mermaid`, `docs/ticket_lifecycle_v3.mermaid`, `docs/ticket_lifecycle_v4.mermaid`, `docs/ticket_lifecycle_v5.mermaid` (current)
8. Requirements source of truth: `../Airtribe_Project_requirements_Repo/trustdesk-capstone/` (problem statement, seed data in `data/`, eval cases)

**Current phase:** v5 milestones (LLD_v5 §8, order V5-1 → V5-27). Standing rule: the full v1-v4 test suite, including eval_005/006/007 and v4's portal-injection adversarial tests, must be green at the end of every v5 milestone; from V5-25 onward the new magic-link-replay adversarial test joins that permanent set.
